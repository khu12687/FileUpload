...
<script>
   function appendItem(title, filename, filecode) {
      const tr = $("<tr>");  
      const td_title = $("<td>").text(title); //textContent 를 정하는 메소드 =
      const td_filename = $("<td>");
      const a_filename = $("<a>").text(filename); //
      a_filename.attr("href", "upload/" + filecode);
      a_filename.attr("target", "_blank");
      const td_delete = $("<td>");
      const a_delete = $("<a>").text("삭제");
      a_delete.attr("href", "delete/" + title);
   ...
   $(function() {
      ...
      $("#list").click(function(){ 
         ...
         // innerHtml 속성에 넣어야 함.
         const tbody = $("tbody").html( ); //파라미터가 없으면 getter
         $("tbody").html(tbody + html); //파라미터가 있으면 setter
      });
   });
   $(function(){
